...
interface SystemKaSelectProps {
  system_kas: SystemKa[];
  handleChange: (key: string, value: SystemKa[]) => void; // Changed from string[] to SystemKa[]
...
export default function SystemKaSelect(props: SystemKaSelectProps) {
  const { selectedIdList, system_kas, handleChange } = props;

  // Convert selectedIdList to the format expected by Autocomplete
  const selectedOptions = React.useMemo(() => {
    return system_kas.filter(ka => 
      selectedIdList.includes(ka.system_ka_number)
    );
  }, [system_kas, selectedIdList]);
  ...
  return (
    <FormControl sx={{ pb: 2 }} fullWidth required>
      <Autocomplete
        multiple
        id="virtualized-system-ka-select"
        options={system_kas}
        value={selectedOptions}
        onChange={handleAutocompleteChange}
        disableCloseOnSelect
        disableListWrap
        getOptionLabel={(option) => option.system_ka_number}
        // Fix: Ensure isOptionEqualToValue works correctly
        isOptionEqualToValue={(option, value) => 
          option.system_ka_number === value.system_ka_number
        }
        ...
        renderInput={(params) => (
          <TextField 
            {...params} 
            label="System KA Selection" 
            placeholder="Select System KAs" 
          />
        )}
